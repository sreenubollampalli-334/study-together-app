...
function MainLayout() {
  ...
  // 🔥 NEW STREAK STATE (REPLACED sessions)
  ...
  useEffect(() => {
    if (email) {
      api.get(`/streak/get?email=${email}`)
  ...
  const updateStreak = async () => {
    ...
    try {
      const res = await api.post(`/streak/update?email=${email}`);
      setStreak(res.data);
    } catch (err) {
      console.error(err);
    }
  ...
  const progress =
    minutes > 0 ? ((minutes * 60 - time) / (minutes * 60)) * 100 : 0;
  ...
  return (
    <>
      {![
        ...
        !location.pathname.startsWith("/chat") && <ChatBot />}
